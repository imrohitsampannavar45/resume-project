...
const Dashboard = () => {
  ...
  const colors = ["#9333ea","#dc2626","#fbbf24", "#818cf8",  "#34d399", "#f87171", "#fbbf24", "#dc2626", "#0284c7", "#16a34a"]
...
<p className='text-2xl font-medium mb-6 bg-gradient-to-r from slate-600 to slate-700 bg-clip-text text-transparent sm:hidden '>Welcome, Ashwani</p>
  ...
  <button className='w-full bg-white sm:max-w-36 h-48 flex flex-col items-center 
    ...
    border-slate-300 group hover:border-purple-500 hover:shadow-lg transition-all duration-300 cursor-pointer'>
      ...
      <p className='text-sm group-hover:text-purple-600 transition-all duration-300'>Upload Exisiting</p>
    </button>
...
return (
  <button key={index} className='relative w-full sm:max-w-36 h-48 flex flex-col items-center justify-center rounded-lg
  gap-2 border group hover:shadow-lg transition-all duration-300 cursor-pointer' style={{background: `linear-gradient(135deg, ${baseColor}10, ${baseColor}40)`,borderColor: baseColor + '40'}}>
...
<PencilIcon className='size-7 p-1.5 hover:bg-white/50 rounded text-slate-700 transition-colors ml-2' />
</div>
