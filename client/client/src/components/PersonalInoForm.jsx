import { Briefcase, Github, Globe, Linkedin, Mail, MapPin, Phone, User } from 'lucide-react'
import React from 'react'

const PersonalInoForm = ({ data, onChange, removeBackground, setRemoveBackground }) => {
    ...
    return (
        <div>
            ...
            <div className='flex items-center gap-2'>
                <label>
                    {data.image ? (
                        <img src={typeof data.image === 'string' ? data.image : URL.createObjectURL(data.image)} alt="user-image" className='
                w-16 h-16 rounded-full object-cover mt-5 ring ring-slate-300 hover:opacity-80' />
                    ) : (
    ...
    return (
        <div key={field.key} className='space-y-1 mt-5'>
            <label className='flex items-center gap-2 text-sm font-medium text-gray-700'>
                ...
                <input type={field.type} value={data[field.key] || ""} onChange={(e)=> handleChange(field.key, e.target.value) } className='
                ...
                placeholder={`Enter Your ${field.label.toLowerCase()}`} required={field.required} name="" id="" />
                </div>
    )
})}
...
export default PersonalInoForm
